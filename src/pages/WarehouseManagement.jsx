...
export default function WarehouseManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let dbWarehouses = [];
      try {
        dbWarehouses = await base44.entities.Warehouse.list();
        setWarehouses(dbWarehouses);
        if (dbWarehouses.length > 0 && !activeWarehouse) {
          setActiveWarehouse(dbWarehouses[0]);
        }
      } catch (err) {
        console.warn("Firestore warehouses query failed.");
      }
  ...
  const totalStockValue = inventory.reduce((acc, curr) => {
    const prod = products.find(p => p.id === curr.productId);
    return acc + (curr.quantity * (prod?.costPrice || 0));
  }, 0);
  const pendingPOs = purchaseOrders.filter(po => po.status !== 'Closed' && po.status !== 'Received').length;
  const avgLeadTime = vendors.length > 0 ? (vendors.reduce((acc, curr) => acc + (parseInt(curr.leadTime) || 3), 0) / vendors.length).toFixed(1) : '2.4';
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-up bg-card/60 backdrop-blur-xl p-6 rounded-2xl border border-border/50 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-1 flex-wrap">
            ...
            <Select value={activeWarehouse?.id || ''} onValueChange={(val) => {
              const wh = warehouses.find(w => w.id === val);
              setActiveWarehouse(wh);
            }}>
              <SelectTrigger className="w-[200px] h-8 text-[11px] font-black uppercase tracking-widest bg-card border-primary/20 rounded-full">
                ...
                {warehouses.length > 0 ? (
                  warehouses.map(wh => (
                    <SelectItem key={wh.id} value={wh.id} className="text-xs font-bold">
                      {wh.country_code ? `${wh.country_code} - ` : ''}{wh.name}
                    </SelectItem>
                  ))
                ) : (
                  <SelectItem value="default" disabled className="text-xs font-bold">Default Warehouse</SelectItem>
                )}
              </SelectContent>
            </Select>
        ...
        <div className="flex items-center gap-2 relative z-10 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 hide-scrollbar">
          {activeTab === 'vendors' && (
            <PermissionGuard module="warehouse" action="create" fallback={null}>
              <Button onClick={() => { resetVendorForm(); setIsVendorOpen(true); }} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl shadow-lg shadow-primary/20 transition-transform active:scale-95 shrink-0">
          ...
          {activeTab === 'po' && (
            <PermissionGuard module="warehouse" action="create" fallback={null}>
              <Button onClick={handleGenerateAutoPOs} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl shadow-lg shadow-primary/20 transition-transform active:scale-95 shrink-0">
          ...
          {activeTab === 'batches' && (
            <PermissionGuard module="warehouse" action="create" fallback={null}>
              <Button onClick={() => { resetBatchForm(); setIsBatchOpen(true); }} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl shadow-lg shadow-primary/20 transition-transform active:scale-95 shrink-0">
      ...
      {/* Global Alert Strip Ticker */}
      <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-2 flex items-center gap-3 overflow-hidden">
        <AlertTriangle className="w-4 h-4 text-red-600 dark:text-red-500 shrink-0 animate-pulse" />
        <span className="text-xs font-bold text-red-600 dark:text-red-500 uppercase tracking-widest shrink-0">Critical Alerts:</span>
        <div className="flex-1 overflow-hidden relative h-4">
          <div className="absolute top-0 left-0 w-full whitespace-nowrap text-xs font-semibold text-red-700/80 dark:text-red-400 animate-ticker">
            {reorderCount > 0 && <span className="mr-8">🚨 {reorderCount} SKU(s) below reorder threshold! Immediate action required.</span>}
            {expiringCount > 0 && <span className="mr-8">⚠️ {expiringCount} Batch(es) expiring within 60 days.</span>}
            {expiredCount > 0 && <span className="mr-8">⛔ {expiredCount} Batch(es) expired and need disposal.</span>}
            {pendingPOs > 0 && <span className="mr-8">⏳ {pendingPOs} Purchase Orders pending completion.</span>}
            {reorderCount === 0 && expiringCount === 0 && expiredCount === 0 && pendingPOs === 0 && <span>✅ All warehouse metrics are optimal. No critical alerts.</span>}
          </div>
        </div>
      </div>

      {/* Enterprise KPI Cards Grid - Row 1 & 2 */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-up" style={{ animationDelay: '0.1s' }}>
        {/* Total SKUs */}
        <Card className="relative overflow-hidden group hover:shadow-lg transition-all duration-300 border-indigo-500/20 bg-indigo-50/30 dark:bg-indigo-950/20">
          <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 transition-transform group-hover:scale-150 duration-500"></div>
          <CardContent className="pt-6 relative z-10">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <p className="text-[10px] font-black text-indigo-700/80 dark:text-indigo-400/80 uppercase tracking-widest">Total SKUs</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-extrabold text-indigo-900 dark:text-indigo-100">{products.length}</span>
                  <div className="flex items-center gap-1 text-[10px] text-indigo-600 dark:text-indigo-400 font-bold bg-indigo-500/10 px-1.5 py-0.5 rounded-full border border-indigo-500/20">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse"></div>
                    Live
                  </div>
                </div>
              </div>
              <div className="p-3 bg-indigo-500/10 rounded-2xl text-indigo-600 dark:text-indigo-400 shadow-sm">
        ...
        <Card className="relative overflow-hidden group hover:shadow-lg transition-all duration-300 border-emerald-500/20 bg-emerald-50/30 dark:bg-emerald-950/20">
          ...
          <CardContent className="pt-6 relative z-10">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <p className="text-[10px] font-black text-emerald-700/80 dark:text-emerald-400/80 uppercase tracking-widest">Total Stock Value</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-extrabold text-emerald-900 dark:text-emerald-100">₹{totalStockValue.toLocaleString('en-IN')}</span>
                </div>
              </div>
        ...
        {/* Active Vendors */}
        <Card className="relative overflow-hidden group hover:shadow-lg transition-all duration-300 border-blue-500/20 bg-blue-50/30 dark:bg-blue-950/20">
          <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 transition-transform group-hover:scale-150 duration-500"></div>
          <CardContent className="pt-6 relative z-10">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <p className="text-[10px] font-black text-blue-700/80 dark:text-blue-400/80 uppercase tracking-widest">{t('warehouse.active_vendors')}</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-extrabold text-blue-900 dark:text-blue-100">{vendors.length}</span>
                </div>
              </div>
              <div className="p-3 bg-blue-500/10 rounded-2xl text-blue-600 dark:text-blue-400 shadow-sm">
          ...
          <CardContent className="pt-6 relative z-10">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                ...
                <div className="flex items-baseline gap-2">
                  <span className={`text-4xl font-extrabold ${reorderCount > 0 ? 'text-red-600 dark:text-red-500' : 'text-amber-600 dark:text-amber-500'}`}>{reorderCount}</span>
        ...
        <Card className="relative overflow-hidden group hover:shadow-lg transition-all duration-300 border-orange-500/20 bg-orange-50/30 dark:bg-orange-950/20">
          ...
          <CardContent className="pt-6 relative z-10">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <p className="text-[10px] font-black text-orange-700/80 dark:text-orange-400/80 uppercase tracking-widest">Expiring Soon</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-extrabold text-orange-600 dark:text-orange-500">{expiringCount}</span>
                  <span className="text-[10px] text-orange-600/70 dark:text-orange-500/70 font-semibold">&lt; 60 days</span>
                </div>
              </div>
        ...
        {/* Pending POs */}
        <Card className="relative overflow-hidden group hover:shadow-lg transition-all duration-300 border-purple-500/20 bg-purple-50/30 dark:bg-purple-950/20">
          <div className="absolute top-0 right-0 w-32 h-32 bg-purple-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 transition-transform group-hover:scale-150 duration-500"></div>
          <CardContent className="pt-6 relative z-10">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <p className="text-[10px] font-black text-purple-700/80 dark:text-purple-400/80 uppercase tracking-widest">Pending POs</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-extrabold text-purple-900 dark:text-purple-100">{pendingPOs}</span>
                </div>
              </div>
              <div className="p-3 bg-purple-500/10 rounded-2xl text-purple-600 dark:text-purple-400 shadow-sm">
        ...
        {/* Average Lead Time */}
        <Card className="relative overflow-hidden group hover:shadow-lg transition-all duration-300 border-teal-500/20 bg-teal-50/30 dark:bg-teal-950/20">
          <div className="absolute top-0 right-0 w-32 h-32 bg-teal-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 transition-transform group-hover:scale-150 duration-500"></div>
          <CardContent className="pt-6 relative z-10">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <p className="text-[10px] font-black text-teal-700/80 dark:text-teal-400/80 uppercase tracking-widest">Avg Lead Time</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-extrabold text-teal-900 dark:text-teal-100">{avgLeadTime}</span>
                  <span className="text-[10px] text-teal-600/70 dark:text-teal-500/70 font-semibold">days</span>
                </div>
              </div>
              <div className="p-3 bg-teal-500/10 rounded-2xl text-teal-600 dark:text-teal-400 shadow-sm">
        ...
        {/* Capacity Used */}
        <Card className="relative overflow-hidden group hover:shadow-lg transition-all duration-300 border-rose-500/20 bg-rose-50/30 dark:bg-rose-950/20">
          <div className="absolute top-0 right-0 w-32 h-32 bg-rose-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 transition-transform group-hover:scale-150 duration-500"></div>
          <CardContent className="pt-6 relative z-10">
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <p className="text-[10px] font-black text-rose-700/80 dark:text-rose-400/80 uppercase tracking-widest">Capacity Used</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-extrabold text-rose-900 dark:text-rose-100">{activeZoneData.occupancy}%</span>
                </div>
              </div>
              <div className="p-3 bg-rose-500/10 rounded-2xl text-rose-600 dark:text-rose-400 shadow-sm">
                          ...
                          return (
                            <tr key={product.id} className={`text-sm transition-colors hover:bg-secondary/40 ${i % 2 === 0 ? 'bg-transparent' : 'bg-secondary/10'}`}>
                              <td className="p-4">
                                <div className="flex items-center gap-3">
                                  {product.image_url ? (
                                    <img src={product.image_url} alt={product.name} className="w-10 h-10 rounded-lg object-cover shrink-0 border border-slate-200 dark:border-slate-800 shadow-sm" />
                                  ) : (
                                    <div className="w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-800/50 flex items-center justify-center shrink-0 border border-slate-200 dark:border-slate-800 shadow-sm">
      ...
      <Dialog open={isVendorOpen} onOpenChange={setIsVendorOpen}>
        <DialogContent className="sm:max-w-[500px] w-[95vw] bg-card p-5 rounded-2xl border border-border shadow-2xl text-foreground max-h-[90vh] overflow-y-auto">
          <DialogHeader>
