      ...
      {!selectedEmployee ? (
        <>
          ...
          {filteredEmployees.length === 0 && (
            <div className="flex flex-col items-center justify-center bg-card/40 backdrop-blur-md border border-border/50 p-12 rounded-2xl shadow-xl text-center space-y-4">
              <Users className="w-16 h-16 text-muted-foreground/30 mb-2" />
              <h3 className="text-xl font-black text-foreground">No employees onboarded yet</h3>
              <p className="text-sm text-muted-foreground max-w-md mx-auto">
                There are currently no employees matching your filters, or you haven't onboarded any staff members yet.
              </p>
              <Button 
                onClick={handleOpenOnboarding} 
                className="mt-4 gap-2 font-bold gold-gradient text-black"
              >
                <UserPlus className="w-4 h-4" /> Onboard First Employee
              </Button>
            </div>
          )}
        </>
        ...
        <DialogContent className="max-w-3xl bg-card border border-border/50 text-xs overflow-y-auto max-h-[85vh] scrollbar-thin">
          ...
          <Tabs value={onboardActiveTab} onValueChange={setOnboardActiveTab} className="space-y-4">
            ...
            <TabsContent value="personal" className="space-y-4 animate-in fade-in duration-300">
              <div className="flex flex-col sm:flex-row gap-4 items-start bg-secondary/5 p-4 rounded-xl border border-border/20">
                <PremiumImageUploader
                  value={onboardForm.photo_url}
                  onChange={(val) => setOnboardForm(prev => ({ ...prev, photo_url: val }))}
                  label="Employee Face Photo"
                  recommendedWidth={300}
                  recommendedHeight={300}
                  maxSizeBytes={2 * 1024 * 1024}
                  maxSizeLabel="2MB"
                  aspectRatio="w-24 h-24 bg-card"
            ...
            <TabsContent value="factory" className="space-y-4 animate-in fade-in duration-300">
              ...
              <div className="border border-border/40 p-4 rounded-xl bg-slate-500/5 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                  {/* Thumbprint Image Upload Box */}
                  <PremiumImageUploader
                    value={onboardForm.thumb_url || ""}
                    onChange={(val) => setOnboardForm(prev => ({ ...prev, thumb_url: val }))}
                    label="Thumbprint Scan Image"
                    recommendedWidth={200}
                    recommendedHeight={200}
                    maxSizeBytes={1 * 1024 * 1024}
                    maxSizeLabel="1MB"
                    aspectRatio="aspect-square bg-card"
                  />
