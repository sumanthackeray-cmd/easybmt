      ...
      <Tabs value={activeRoleId} onValueChange={setActiveRoleId} className="w-full">
        ...
        <div className="md:hidden mb-4">
          <select
            ...
            className="w-full bg-card border border-border rounded-lg px-3 py-2.5 text-xs font-bold focus:outline-none shadow-sm text-foreground appearance-none"
            ...
            {roles
              ...
              .map((role) => (
                <option key={role.id} value={role.id}>
                  {role.role_name} (L{role.hierarchy_level})
                </option>
              ))}
          </select>
        </div>

        <TabsList className="hidden md:flex bg-secondary mb-4 flex-wrap h-auto gap-1">
          {roles
            .sort((a, b) => a.hierarchy_level - b.hierarchy_level)
            .map((role) => (
              <TabsTrigger 
