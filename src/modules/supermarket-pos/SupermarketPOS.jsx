...
import { printReceipt } from "@/lib/pos-print-service";
...
import { generateThermalHTML, downloadInvoicePDF, shareInvoiceViaWhatsApp, generateAndUploadInvoicePDF } from "@/lib/pdf-share-utils";
 ...
 */
function Terminal({ activeSession, products, activeOffers, loyaltyCards, customers, onCloseShift, companyName }) {
  ...
  useEffect(() => {
    if (shopSettings?.barcode_quick_billing && scanInput.trim().length > 3) {
  ...
  }, [scanInput, shopSettings?.barcode_quick_billing, products]);

  // Scan handler (fallback for Enter key)
  ...
  const addToCart = (product, quantity) => {
    setCart(prevCart => {
      ...
      const currentQty = idx > -1 ? prevCart[idx].qty : 0;
      let nextQty = currentQty;
      
      if (product.requires_scale || product.is_weighed) {
        nextQty = Number((currentQty + quantity).toFixed(3));
      } else {
        nextQty += quantity;
      }

      if (product.stock !== undefined && product.stock !== null && nextQty > product.stock) {
        toast.error(`Insufficient stock. Only ${product.stock} left in stock for ${product.name}.`);
        return prevCart;
      }
      ...
      if (idx > -1) {
        ...
        return [...prevCart, { ...product, qty: nextQty }];
      }
    });
  };
      ...
      return prevCart.map(item => {
        if (item.id === id) {
          ...
          if (change > 0 && item.stock !== undefined && item.stock !== null && newQty > item.stock) {
            toast.error(`Insufficient stock. Only ${item.stock} left in stock for ${item.name}.`);
            return item;
          }
  ...
  const cartTotals = useMemo(() => {
    const totals = calculateCartOffers(cart, activeOffers, !!activeMember);
    if (shopSettings?.roundoff_total !== false) {
      totals.grandTotal = Math.round(totals.grandTotal);
    }
    return totals;
  }, [cart, activeOffers, activeMember, shopSettings?.roundoff_total]);
  ...
  const handleCheckoutSubmit = async () => {
    ...
    try {
      ...
      const invoicePayload = {
        ...
        customer_name: selectedCustomer ? selectedCustomer.name : (activeMember ? activeMember.name : "Walk-in Customer"),
        customer_phone: selectedCustomer ? selectedCustomer.phone : (activeMember ? activeMember.phone : ""),
        customer_id: selectedCustomerId === "walk-in" ? "" : selectedCustomerId,
      ...
      // Decrement inventory stock safely using local-first updateInventory
      for (const item of cart) {
        try {
          const branchId = localStorage.getItem("branch_id") || "main";
          await updateInventory(item.id, branchId, -item.qty, "pos_sale");
          
          // Update global product catalog stock for simple B2C UI
          ...
          console.error(`Error updating inventory stock for product ${item.id}:`, err);
        }
      }
      queryClient.invalidateQueries({ queryKey: ["products"] });
      ...
      if (isMounted.current) {
        ...
        setWhatsappNumber(selectedCustomer ? (selectedCustomer.phone || "") : (activeMember?.phone || ""));
        ...
        if (autoPrint) {
          try {
            printReceipt(createdInvoice, shopSettings).catch(err => {
              console.warn("Background auto-print failed:", err);
            });
          } catch (printErr) {
            console.warn("Auto-print dispatch failed:", printErr);
          }
        }
      }
  ...
  const handleSendWhatsApp = async () => {
    ...
    await shareInvoiceViaWhatsApp(lastInvoice, shopSettings, whatsappNumber);
  };
  ...
  // ULTRA-PERFORMANCE: Debounce search term
  useEffect(() => {
  ...
  // ULTRA-PERFORMANCE: Reset visible count on category change
  useEffect(() => {
    setVisibleCount(50);
  }, [activeCategory]);
    ...
    if (debouncedSearchTerm) {
      result = result.filter(p => {
        const matchSearch = p.name?.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) || 
                            p.plu_code?.includes(debouncedSearchTerm);
        return matchSearch;
      });
    }
    return result;
  ...
  return (
    ...
    <div className="flex flex-row h-screen bg-slate-50 dark:bg-slate-950 select-none overflow-hidden">
      ...
      <main className="flex flex-1 overflow-hidden">
        ...
        {/* Quick Access Item Grid (Virtualized) */}
        <div 
        ...
        >
          {filteredProducts.length === 0 ? (
              ...
              <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-2 pb-6">
                {filteredProducts.slice(0, visibleCount).map(p => {
                  ...
                  >
                    {/* CART BADGE */}
                    {cartQty > 0 && (
                      <div className="absolute -top-2 -right-2 bg-emerald-500 text-white text-[11px] font-black w-6 h-6 flex items-center justify-center rounded-full shadow-md z-10 animate-fade-in border border-white/50 dark:border-slate-800">
                        {cartQty}
                      </div>
                    )}
                    ...
                    <div className="absolute top-2 left-2 z-10">
                      <span className={cn(
                        "text-[9px] font-black px-1.5 py-0.5 rounded-full shadow-sm",
                        (p.stock !== undefined && p.stock !== null && p.stock <= 5) ? "bg-red-100 text-red-700 border border-red-200" : "bg-emerald-100 text-emerald-700 border border-emerald-200"
                      )}>
                        {p.stock !== undefined && p.stock !== null ? `${p.stock} in stock` : 'N/A'}
                      </span>
                    </div>

                    {/* PLU Tag */}
                    {p.plu_code && (
                      <span className="absolute top-1 right-2 text-[8px] font-mono text-slate-400 group-hover:text-blue-600 font-black z-10 bg-white/80 dark:bg-slate-900/80 px-1 rounded backdrop-blur-sm">
                        PLU {p.plu_code}
                      </span>
                    )}

                    {/* Image Block */}
                    <div className="w-full h-24 bg-slate-50 dark:bg-slate-800/50 rounded-xl mb-2 flex items-center justify-center overflow-hidden border border-slate-100 dark:border-slate-800">
                      {p.image_url ? (
                    ...
                    <div className="space-y-1 w-full mt-1">
                      <p className="text-[9px] text-muted-foreground uppercase font-black tracking-wider leading-none truncate">
                        {p.category}
                      </p>
        ...
        <section className={cn(
          ...
          <div className="flex flex-col gap-1.5 p-2 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800 relative">
            ...
            <select
              ...
              className="w-full h-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold px-2 rounded-md focus:outline-none focus:border-amber-400"
            >
                                  ...
                                  <div className="flex items-center text-[10px] text-slate-400 font-mono">
                                    ₹<input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        setCart(prev => prev.map(c => c.id === item.id ? { ...c, rate: val } : c));
                                      }}
                                      className="w-12 h-5 text-center font-mono text-[10px] rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 ml-0.5"
                                    />
                                  </div>
                                ) : (
                                  <span className="text-[10px] text-slate-400 font-mono">₹{item.mrp || item.rate}</span>
                                )}
                                {discount > 0 && (
                            ...
                            <div className="flex items-center justify-center gap-1">
                              ...
                              >
                                ...
                                <input
                                  ...
                                  className="w-12 h-6 text-center font-mono text-xs font-black rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100"
                                />
                                <button
                                  onClick={() => updateQty(item.id, 1)}
                                  className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-800 font-bold hover:bg-slate-200 flex items-center justify-center text-xs"
      ...
      <Dialog open={showCheckout} onOpenChange={setShowCheckout}>
        <DialogContent className="max-w-md w-full max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-6 rounded-3xl border border-border">
          ...
          <div className="space-y-5 pt-3">
            ...
            <DialogFooter className="pt-2 flex items-center justify-between gap-2 flex-col sm:flex-row w-full">
              <div className="flex items-center gap-2 mr-auto text-xs font-bold text-slate-600 dark:text-slate-400 select-none py-2 sm:py-0">
                <input 
                  type="checkbox" 
                ...
                />
                <label htmlFor="autoPrintCheckbox" className="cursor-pointer">Auto-Print Receipt</label>
              </div>
              <div className="flex gap-2 w-full sm:w-auto">
                <Button 
                  variant="outline" 
                  onClick={() => setShowCheckout(false)}
                  className="flex-1 sm:flex-initial font-bold h-11 min-w-[100px]"
                >
                  Go Back
                </Button>
                <Button
                  onClick={handleCheckoutSubmit}
                  className="flex-1 sm:flex-initial bg-blue-600 hover:bg-blue-700 font-bold h-11 text-white min-w-[140px]"
                >
                  Complete Sale
                </Button>
              </div>
      ...
      <Dialog open={showReceipt} onOpenChange={setShowReceipt}>
        <DialogContent showClose={false} className={`bg-white text-slate-950 p-0 gap-0 rounded-2xl overflow-hidden transition-all duration-200 flex flex-col ${selectedPrintSize === "80mm" ? "sm:max-w-[420px]" : "sm:max-w-[360px]"} max-h-[90vh] mt-4 sm:mt-0 print:max-h-none print:overflow-visible print:bg-transparent print:border-none print:shadow-none`}>
          {/*  Top controls bar  */}
          <div className="flex gap-2 px-4 pt-5 pb-3 border-b border-gray-100 shrink-0 items-center justify-between">
          ...
          <div className={`flex-1 overflow-y-auto min-h-0 px-4 py-3 bg-[#1a1a2e] thermal-receipt-print-area ${selectedPrintSize === "80mm" ? "printer-80mm" : "printer-58mm"} print:bg-transparent print:p-0 print:overflow-visible print:block`}>
             {lastInvoice && (
               <div 
                 ...
                 dangerouslySetInnerHTML={{ __html: generateThermalHTML(lastInvoice, shopSettings, selectedPrintSize) }} 
               />
             )}
          </div>
                 ...
                 <Button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const printSettings = { ...shopSettings, printer_size: selectedPrintSize };
                      await printReceipt(lastInvoice, printSettings);
